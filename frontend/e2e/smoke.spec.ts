...
test.describe('about', () => {
  test('renders the word, the directors and honest pricing', async ({ page }) => {
    ...
    // the fee is its own block: three rows, the seeded numbers, the kicker under them
    const fee = page.locator('.fee')
    await expect(fee).toContainText('R 250')
    await expect(fee).toContainText('R 100')
    await expect(fee).toContainText('R 1000')
    await expect(fee.locator('.note')).toContainText(/monthly or annual membership/)
    await expect(page.getByText(/Anyone can join/)).toBeVisible()
    await expect(page.locator('.open-door')).toContainText(/write to us anyway/)
